...
declare [named filters](config-filter.md) in the [configuration file](config.md)
for the same set of options you use frequently.
...
[See the `inline` command alias example](config-alias.md) for more explanation.
...
To set the default strategy, you can declare a [custom alias](config-alias.md):
...
Finally, you can use glob patterns to match multiple tags. This is particularly
useful if you use a separator (e.g. `/`) to group multiple tags under a parent
tag.
...
selection is handled by [`fzf`](tool-fzf.md) which brings a powerful fuzzy
